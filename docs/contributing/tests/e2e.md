...
Argus uses end-to-end tests to verify the frontend works correctly against a real backend. These tests use Playwright to drive a real browser against a containerized Argus stack that pytest brings up — the same fixture graph integration tests use, with an extra HTTP seeder container for per-spec telemetry seeding.
...
To spin up the backend stack (Argus, ClickHouse, Postgres, Zookeeper, Zeus mock, gateway mock, seeder, migrator-with-web) and keep it running:
...
The `--with-web` flag builds the frontend into the Argus container — required for E2E. The build takes ~4 mins on a cold start.
...
Playwright drives a real browser (Chromium / Firefox / WebKit) against the running Argus frontend. The backend is brought up by the same pytest fixture graph integration tests use, so both suites share one source of truth for container lifecycle, license seeding, and test-user accounts.
...
- `--with-web` — build the frontend into the Argus container. **Required for E2E**; integration tests don't need it.

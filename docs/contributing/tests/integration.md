...
Argus uses integration tests to verify that different components work together correctly in a real environment. These tests run against actual services (ClickHouse, PostgreSQL, Argus, Zeus mock, Keycloak, etc.) spun up as containers, so suites exercise the same code paths production does.
...
This command will:
- Start all required services (ClickHouse, PostgreSQL, Zookeeper, Argus, Zeus mock, gateway mock)
...
│   ├── postgres.py
│   ├── argus.py            # Argus-backend container
│   ├── sql.py
│   ├── types.py
...
Test suites should target functional domains or subsystems within Argus. When designing a test suite, consider these principles:
...
def test_version(argus: types.Argus) -> None:
    response = requests.get(
        argus.self.host_configs["8080"].get("/api/v1/version"),
        timeout=2,
    )
    logger.info(response)
```

We have written a simple test which calls the `version` endpoint of the Argus backend. **To run just this function, run the following command:**
...
def test_user_registration(argus: types.Argus) -> None:
    """Test user registration functionality."""
    response = requests.post(
        argus.self.host_configs["8080"].get("/api/v1/register"),
        json={
            "name": "testuser",
            "orgId": "",
            "orgName": "test.org",
...
- `--schema-migrator-version` — Argus schema migrator version (default: `v0.144.2`)
...
- **Clean up test data** between tests in the same suite to avoid interference — or rely on a fresh Argus container if you need full isolation.

...
.PHONY: py-test-setup
py-test-setup: ## Bring up the shared Vigil backend used by integration and e2e tests
	@cd tests && uv run pytest --basetemp=./tmp/ -vv --reuse --capture=no integration/bootstrap/setup.py::test_setup

.PHONY: py-test-teardown
py-test-teardown: ## Tear down the shared Vigil backend
...
.PHONY: vigil-run
vigil-run: ## Run the Vigil control plane on :8080
	go run ./cmd/vigil-server

.PHONY: vigil-test
vigil-test: ## Run the Vigil Go test suite under the race detector
	go test -race -count=1 ./pkg/query-service/vigil/...

.PHONY: demo
demo: ## Run the seven-scene Vigil demonstration
	./demo/run_demo.sh

.PHONY: vigil-verify
vigil-verify: ## Run the end-to-end verification against a running server

...
Handlers in Argus are responsible for exposing module functionality over HTTP. They are thin adapters that:
...
The parent keeps its `JSONSchemaOneOf()` (the `oneOf` itself) and *additionally* tags it via `PrepareJSONSchema` with the `x-argus-discriminator` extension; `argus.attachDiscriminators` then promotes that marker to a real OpenAPI 3 `discriminator` (and strips the duplicate parent properties) after reflection.
...
func (Plugin) PrepareJSONSchema(s *jsonschema.Schema) error {
    ...
    s.ExtraProperties["x-argus-discriminator"] = map[string]any{
        "propertyName": "kind",
        "mapping": map[string]string{
            "argus/Foo": "#/components/schemas/FooVariant",
        },
    }
    return nil
}
